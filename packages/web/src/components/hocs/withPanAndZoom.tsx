...
export function withPanAndZoom(Child, options: Options) {
  return class PanAndZoom extends React.Component {
    ...
    startMove(clientX, clientY) {
      if (!options.doPanX && !options.doPanY) return
      this.setState({
        translating: {
          initialCursor: [options.doPanX ? clientX : 0, options.doPanY ? clientY : 0],
    ...
    move(clientX, clientY) {
      const x =
        this.state.translating.initialWorld[0] +
        (options.doPanX ? clientX : 0) -
        this.state.translating.initialCursor[0]
      const y =
        this.state.translating.initialWorld[1] +
        (options.doPanY ? clientY : 0) -
        this.state.translating.initialCursor[1]
    ...
    scale(newScale, centerX, centerY) {
      if (!options.doZoom) return
      const rect = this.containerRef.current.getBoundingClientRect()
      const p = [centerX - rect.left, centerY - rect.top]
      const pWorld = this.mapScreenToWorld([centerX, centerY]) as any
